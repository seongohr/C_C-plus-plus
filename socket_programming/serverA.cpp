...
void printMSG_bootup() {
    cout << "The Server A is up and running using UDP on port "
         << MY_PORT
         << endl
         << endl;
}
...
void printMSG_received(char mapId) {
    cout << "The Server A has received input for finding graph of map "
         << mapId
         << endl
         << endl;
}
...
void printMSG_noMap(string mapId) {
    cout << "The Server A does not have the required graph id "
         << mapId
         << endl
         << endl;
}
...
void printMSG_sent(bool hasMap) {
    if (hasMap == false) {
        cout << "The Server A has sent \"Graph not Found\" to AWS\n";
    }
    else {
...
int main(void)
{
    bool hasMap; // true if mapID exists in the map file
    map<string, string> map; // where the file data read will be stored
