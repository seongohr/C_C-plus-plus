...
class Table {
 ...
 private:
   ...
   ListType *data;
   int numOfEntry; //number of entries in the table
   int nonEmptyBucket; //number of non empty bucket in the table
   int * numOfChains; //track the length of each linkedlist in each bucket in the table

   //initialize the Table(linkedlist array) with NULL
   void initialization();
   
   //initialize an array with 0
   void intArrInitialization();

   //compute the number of longest chain in the array and returns it
